...
- `src/scorched_earth/roe_edit.py` - pure model for the **interactive ROE editor** (#10): the wired rules as a flat list of `controls` (2 cycles + a toggle per known job type x2) plus an `apply(roe, index, direction)` reducer and `save` (preserves freeform keys). Shared by the terminal editor (`scorch roe <repo>` opens a curses arrow-key list; `--json`/non-tty falls back to the JSON print) and, later, an html frontend. Only exposes fields the advisor/runner actually consume - no dead toggles.
