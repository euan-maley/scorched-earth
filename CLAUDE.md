...
- `src/scorched_earth/roe_edit.py` - pure model for the **interactive ROE editor** (#10): the wired rules as a flat list of `controls` (now: run_mode + auto-run DEFCON + run-cap cycles, attended-branch + auto-advise toggles, roadblock idle-timeout cycle, one toggle per known job type x2) plus an `apply(roe, index, direction)` reducer and `save` (preserves freeform keys, e.g. `context_cmd`). Shared by the terminal editor (`scorch roe <repo>` opens a curses arrow-key list; `--json`/non-tty falls back to the JSON print) and, later, an html frontend. Only exposes fields the advisor/runner actually consume - no dead toggles.
